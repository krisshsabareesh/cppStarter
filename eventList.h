...
void search(Node *h) {
	...
	while(temp != NULL) {
		if (temp -> event.time == t) {
			flag = 1;
			std::cout << "Event described as " << temp -> event.description << " found at " << temp -> event.time << std::endl;
		}
		temp = temp -> next;
	}
	if(!flag)
		std::cout <<"Event search failed to find any event at " << t << std::endl;
}
...
void display(Node *h) {
	...
	Node *temp = h;
	while(temp != NULL) {		
		std::cout <<"Event time is : " << temp->event.time <<  ", Event described as : " << temp->event.description << std::endl;
		temp = temp -> next;
	}
}
